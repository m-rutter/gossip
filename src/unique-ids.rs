            ...
            GeneratePayload::Init { .. } => {
                let reply = Message {
                    ...
                    body: Body {
                        ...
                        payload: GeneratePayload::InitOk {},
                    },
                };
                Some(reply)
            }
            GeneratePayload::InitOk => None,
...
fn main() -> anyhow::Result<()> {
    ...
    let inputs = Deserializer::from_reader(stdin).into_iter::<Message<GeneratePayload>>();

    let mut node = GenerateNode { id: 0 };
